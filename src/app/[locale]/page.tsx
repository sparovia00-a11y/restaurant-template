...
export default async function Home({
  ...
  return (
    <main>
      ...
      <section className="relative h-[70vh] flex items-end justify-center pb-16 overflow-hidden bg-[var(--color-text-primary)]/90">
        <div
      ...
      <section className="px-6 md:px-16 py-20">
        ...
        <TypewriterText
          ...
          className="max-w-2xl mx-auto text-center text-[var(--color-text-muted)] mb-12"
        />
      ...
      <section className="px-6 md:px-16 py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-[var(--color-surface-border)]">
          {content.featuredMenu.dishes.map((dish, i) => (
            ...
            >
              ...
              <div className="p-5">
                <p className="text-[11px] uppercase tracking-wide text-[var(--color-accent)]/70 mb-2">
                  {dish.category}
                </p>
                ...
                <div className="border-t pt-3 flex items-center justify-between">
                  <span className="text-[var(--color-accent)] text-sm">{dish.price}</span>
      ...
      >
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <RevealOnScroll>
            ...
            <div className="w-8 h-px mb-6" style={{ backgroundColor: "var(--color-accent)" }} />
            <TypewriterText
        ...
        <div className="flex flex-wrap justify-center gap-x-10 gap-y-8">
          {content.awards.map((award, i) => (
            <RevealOnScroll key={award.id} delay={i * 80} className="flex flex-col items-center gap-3 w-28">
              <div className="w-14 h-14 rounded-full border border-[var(--color-accent)]/30 flex items-center justify-center text-[var(--color-accent)]/70">
      ...
      {googleReviews && googleReviews.reviews.length > 0 && (
        <section className="px-6 md:px-16 py-20 text-center">
          ...
          <div className="flex items-center justify-center gap-2 mb-14">
            <div className="text-[var(--color-accent)] flex">
          ...
          <RevealOnScroll className="flex gap-6 overflow-x-auto snap-x snap-mandatory pb-4 px-1 max-w-6xl mx-auto scroll-smooth [&::-webkit-scrollbar]:hidden">
            {googleReviews.reviews.map((item) => (
              ...
              >
                <div className="text-[var(--color-accent)] mb-4 flex">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      size={14}
                ...
                <p className="font-serif italic text-[var(--color-text-primary)] leading-relaxed mb-6 line-clamp-6">
                ...
                <div className="flex items-center gap-3">
                  {item.authorPhoto ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-[var(--color-accent-soft)] flex items-center justify-center font-serif text-[var(--color-accent)]">
                      {item.author.charAt(0)}
                    </div>
                  )}
                  <div>
                    <p className="text-sm font-medium">{item.author}</p>
                    <p className="text-xs text-[var(--color-text-muted)]">{item.relativeTime}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          {googleReviews.mapsUri && (
            <a
              ...
              className="inline-block mt-10 text-xs uppercase tracking-wide underline underline-offset-4 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]"
            >
      ...
      <section className="grid md:grid-cols-2 gap-0">
        ...
        <RevealOnScroll delay={150} className="flex items-center p-10 md:p-16 bg-[var(--color-text-primary)] text-white">
          <div>
            <h2 className="font-serif text-2xl mb-4">
      ...
      <section className="px-6 md:px-16 py-20 text-center">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-3xl mx-auto">
          <RevealOnScroll
            ...
            style={{ backgroundColor: "var(--color-surface)", borderColor: "var(--color-surface-border)" }}
          >
            <div className="w-12 h-12 rounded-full border border-[var(--color-accent)]/30 flex items-center justify-center text-[var(--color-accent)]/70">
      ...
      {/* Footer */}
      <footer className="bg-[var(--color-text-primary)] text-white px-6 md:px-16 py-14">
